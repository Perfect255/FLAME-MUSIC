...
from Flame.data import RAID, REPLYRAID, FLAMESPAM, PORMS, STICKERS, SHAYRI
...
@BOT.on(events.NewMessage(incoming=True, pattern=r"\%shang(?: |$)(.*)" % hl))
async def hang(e):
    usage = f"**MODULE NAME : HANG SPAM** \n\n Cmd : `{hl}hang <count>`"
    if e.sender_id in SUDO_USERS:
        Perfect = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        if len(Perfect) == 1:
            counter = int(Perfect[0])
...
@BOT.on(events.NewMessage(incoming=True, pattern=r"\%spackspam(?: |$)(.*)" % hl))
